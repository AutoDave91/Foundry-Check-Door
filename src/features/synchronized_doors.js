...
export function onRederWallConfig(wallConfig, html, data) {
	if (game.settings.get(settingsKey, "synchronizedDoors") && data.data.door) {
		...
		const synchronizedSettings = `
			<p class="notes">${game.i18n.localize("door-check.ui.synchronizedDoors.description")}</p>
			<div class="form-group">
				<label for="synchronizationGroup">${game.i18n.localize(
			"door-check.ui.synchronizedDoors.groupName",
		)}</label>
				<input type="text" name="synchronizationGroup"/>
			</div>
			<div class="form-group">
				<label for="synchronizeSecretStatus">${game.i18n.localize(
			"door-check.ui.synchronizedDoors.synchronizeSecretStatus",
		)}</label>
		...
		const doorcheckData = data.object.flags.doorcheck;
		...
		input("synchronizationGroup").prop("value", doorcheckData?.synchronizationGroup);
		input("synchronizeSecretStatus").prop("checked", doorcheckData?.synchronizeSecretStatus);
...
export async function onWallConfigUpdate(event, formData) {
	...
	const updateData = { flags: { doorcheck: { synchronizationGroup: formData.synchronizationGroup } } };
	...
	if (formData.synchronizationGroup) {
		...
		updateData.flags.doorcheck.synchronizeSecretStatus = synchronizeSecretStatus;
		...
		const doorInGroup = Util.findInAllWalls(wall => {
			...
			if (wall.flags.doorcheck?.synchronizationGroup !== formData.synchronizationGroup)
				return false;
...
export function updateSynchronizedDoors(updateData, synchronizationGroup) {
	...
	let scenes = Util.filterAllWalls(
		wall => wall.door && wall.flags.doorcheck?.synchronizationGroup === synchronizationGroup,
	);
